...
func NewAdapterByDBUseTableName(db *gorm.DB, databaseName string, prefix string, tableName string) (*Adapter, error) {
	if len(tableName) == 0 {
...
func NewAdapterByMulDb(dbPool DbPool, dbName string, prefix string, tableName string) (*Adapter, error) {
	...
	return NewAdapterByDBUseTableName(dbPool.source, dbName, prefix, tableName)
}
...
func NewAdapterByDBWithCustomTable(db *gorm.DB, t interface{}, tableName ...string) (*Adapter, error) {
	...
	return NewAdapterByDBUseTableName(db.WithContext(ctx), curTableName, "", curTableName)
}
...
func (a *Adapter) LoadPolicy(model model.Model) error {
	...
	//	return err
	//}
	tableName := a.tablePrefix+"_"+a.tableName
	if a.databaseName != "" {
		tableName = a.databaseName + "." + tableName
	}
	if err := a.db.Raw("select * from "+tableName).Order("ID").Find(&lines).Error; err != nil {
		return err
	}
