...
type AddressInsertReq struct {
	...
	ReceiverName            string `json:"receiverName" comment:"收货姓名" vd:"@:len($)>0;msg:'收获人必填'"`
	DetailAddress           string `json:"detailAddress" comment:"详细地址" vd:"@:len($)>0;msg:'详情地址必填'"`
	PostalCode              string `json:"postalCode" comment:"邮政编码"`
	CellPhone               string `json:"cellPhone" comment:"收件人手机号码" vd:"@:len($)==0||regexp('^1[3-9]\\d{9}$');msg:'手机号必填或不符合规范'"`
	Telephone               string `json:"telephone" comment:"座机号码"`
	IsDefault               int    `json:"isDefault" comment:"是否默认(1默认)" vd:"@:$==0||in($,0,1);msg:'是否默认必填[0,1]'"`
	CountryId               int    `json:"countryId" comment:"国家ID"`
	CountryName             string `json:"countryName" comment:"国家名称"`
	ProvinceName            string `json:"provinceName" comment:"省份名称" vd:"@:len($)>0;msg:'省份名称必填'"`
	ProvinceId              int    `json:"provinceId" comment:"省份ID" vd:"@:$>0;msg:'省份ID必填'"`
	CityId                  int    `json:"cityId" comment:"城市ID" vd:"@:$>0;msg:'城市ID必填'"`
	CityName                string `json:"cityName" comment:"城市名称" vd:"@:len($)>0;msg:'城市名称必填'"`
	AreaId                  int    `json:"areaId" comment:"区域ID" vd:"@:$>0;msg:'区域ID必填'"`
	AreaName                string `json:"areaName" comment:"区域名称" vd:"@:len($)>0;msg:'区域名称必填'"`
	...
	AddressType             int    `json:"addressType" comment:"地址类型（1收货地址，2发票地址,3产品使用地址，4通用）" vd:"@:$>0 && in($,1,2,3,4);msg:'地址类型必填'"`
	IsDeadlineReceivingAddr int    `json:"isDeadlineReceivingAddr" comment:"是否账期收货地址"`
	Email                   string `json:"email" comment:"邮箱（用于发送出库提醒）" vd:"@:len($)==0||regexp('^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(.[a-zA-Z0-9_-]+)+$');msg:'邮箱不符合规范，请检查'"`
...
func (s *AddressInsertReq) Generate(model *models.Address) {
	...
	model.IsDeadlineReceivingAddr = s.IsDeadlineReceivingAddr
	model.Email = s.Email
...
type AddressDefaultReq struct {
	Id          int `uri:"id" comment:"编号" vd:"$>0"` // 编号
	UserId      int `json:"userId" comment:"用户ID"`   // 当前用户ID
	Cancel      int `json:"cancel" comment:"是否取消默认"` // 是否取消 1-取消 0-设置默认
	AddressType int `json:"addressType" comment:"地址类型（1收货地址，2发票地址,3产品使用地址，4通用）" vd:"in($,1,2)"`
}

func (s *AddressDefaultReq) GetId() interface{} {
	return s.Id
}
