...
func (e *OperateLogs) InsertItem(tx *gorm.DB) error {
	...
	err := tx.Table(wcPrefix+"."+logType.TableName()).Where("model_name = ?", e.ModelName).Where("type = ?", e.Type).Take(&logType).Error
	if err == nil {
	...
	//数据库操作
	if err := tx.Table(wcPrefix + "." + e.TableName()).Create(e).Error; err != nil {
		return err
	}
	return nil
}
