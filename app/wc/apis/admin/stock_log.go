...
// @Success 200 {object} response.Response{data=response.Page{list=[]dto.StockLogGetPageResp}} "{"code": 200, "data": [...]}"
// @Router /api/v1/wc/admin/stock-log [get]
// @Security Bearer
func (e StockLog) GetPage(c *gin.Context) {
	req := dto.StockLogGetPageReq{}
	s := service.StockLog{}
	err := e.MakeContext(c).
		MakeOrm().
		Bind(&req).
	...
	list := make([]dto.StockLogGetPageResp, 0)
	var count int64

	err = s.GetPage(&req, p, &list, &count)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("获取库存日志失败，\r\n失败信息 %s", err.Error()))
		return
	}
...
// @Router /api/v1/wc/admin/stock-log/export [get]
// @Security Bearer
func (e StockLog) Export(c *gin.Context) {
	req := dto.StockLogGetPageReq{}
	s := service.StockLog{}
	err := e.MakeContext(c).
		MakeOrm().
	...
	p := actions.GetPermissionFromContext(c)
	outData := &[]dto.StockLogGetPageResp{}
	...
	err = excelApp.ExportExcelByMap(e.Context, title, exportData, "库存变动记录-导出", "A1")
	if err != nil {
		e.Error(500, err, fmt.Sprintf("库存变动记录导出失败，\r\n失败信息 %s", err.Error()))
		return
	}
