...
func (e SysTable) GetByTableName(c *gin.Context) (tools.SysTables, error) {
	e.Context = c
	var data tools.SysTables
	log := e.GetLogger()
	db, err := e.GetOrm()
...
func (e SysTable) GetByTableNameCustom(c *gin.Context) (tools.SysTables, error) {
	e.Context = c
	var data tools.SysTables
	log := e.GetLogger()
	db, err := e.GetOrm()
...
func (e SysTable) Insert(c *gin.Context) {
	...
	for i := 0; i < len(tablesList); i++ {
		fmt.Println("tablesList------", tablesList)
...
func (e SysTable) InsertCustom(c *gin.Context) error {
	e.Context = c
	log := e.GetLogger()
	db, err := e.GetOrm()
	if err != nil {
...
func genTableInit(tx *gorm.DB, tablesList []string, i int, c *gin.Context) (tools.SysTables, error) {
	...
	packageName := config.ExtendConfig.(*ext.Extend)
	if packageName.Module == "" {
		packageName.Module = "admin"
	}
	data.PackageName = packageName.Module
