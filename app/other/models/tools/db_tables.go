...
func (e *DBTables) GetPage(tx *gorm.DB, pageSize int, pageIndex int) ([]DBTables, int, error) {
	...
	if config2.DatabasesConfig["base"].Driver == "mysql" {
		table = tx.Table("information_schema.tables")
		table = table.Where("TABLE_NAME not in (select table_name from `" + global.GetTenantAdminDBNameWithDB(tx) + "`.sys_tables) ")
