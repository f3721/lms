...
func SetDBOperLog(c *gin.Context, clientIP string, statusCode int, reqUri string, reqMethod string, latencyTime time.Duration, body string, result string, status int) {
	if global.GetTenantDBPrefixWithContext(c) == "base" {
		return
	}
	...
	//q := sdk.Runtime.GetMemoryQueue(c.Request.Host) //默认根据host，需要改造
	q := sdk.Runtime.GetMemoryQueue(global.GetTenantDBPrefixWithContext(c))
